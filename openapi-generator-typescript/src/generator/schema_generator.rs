                ...
                if let Some(additional_props) = &obj_schema.additional_properties {
                    match additional_props.as_ref() {
                        AdditionalProperties::RefOr(schema_ref) => {
                            let value_type = self.map_ref_or_schema_to_type(schema_ref, context);
        ...
        match schema_type {
            ...
            SchemaType::Array(types) => {
                ...
                if types.len() == 1 {
                    ...
                    for openapi_type in types {
                        let ts_type = self
                            .map_schema_type_to_primitive(&SchemaType::Type(openapi_type.clone()));
