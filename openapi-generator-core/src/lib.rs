...
use openapi_generator_transforms::{TransformPipeline, passes::{NamingConventionPass, ReferenceResolutionPass, ValidationPass, NamingConvention}};
...
impl CodeGenerator {
    ...
    pub fn generate_from_file<P: AsRef<std::path::Path>>(&self, input_path: P, output_dir: P, languages: &[String]) -> Result<(), error::Error> {
        tracing::info!("Parsing OpenAPI specification from: {:?}", input_path.as_ref());
        let mut openapi = parse_file_with_validation(input_path)
            .context(error::ParseSnafu)?;

        tracing::info!("Applying transformations");
        self.transform_pipeline.transform(&mut openapi)
        ...
        for language in languages {
            match language.as_str() {
                "typescript" | "ts" => {
                    ...
                    let code = self.typescript_generator.generate(&openapi)
                        .map_err(|e| error::Error::Generate { source: Box::new(e) })?;
                    self.write_output(&output_dir, "typescript", &code)?;
                }
                "rust" => {
                    tracing::info!("Generating Rust code");
                    let code = self.rust_generator.generate(&openapi)
                        .map_err(|e| error::Error::Generate { source: Box::new(e) })?;
                    self.write_output(&output_dir, "rust", &code)?;
                }
                _ => {
                    return Err(error::Error::UnsupportedLanguage { language: language.clone() });
                }
            }
        }
    ...
    fn write_output<P: AsRef<std::path::Path>>(&self, output_dir: P, language: &str, code: &str) -> Result<(), error::Error> {
        let output_path = output_dir.as_ref().join(format!("{}.generated", language));
        std::fs::write(&output_path, code)
            .context(error::WriteOutputSnafu { path: output_path.to_string_lossy().to_string() })?;
